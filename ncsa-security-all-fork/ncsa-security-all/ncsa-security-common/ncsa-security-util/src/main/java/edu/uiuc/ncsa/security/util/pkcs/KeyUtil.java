...
public class KeyUtil {
    ...
    public static String toPKCS1PEM(PrivateKey privateKey) throws IOException {
        ...
        return PEMFormatUtil.delimitBody(Base64.encodeBase64String(bytes), BEGIN_RSA_PRIVATE_KEY, END_RSA_PRIVATE_KEY);
/*

        ByteArrayInputStream inStream = new ByteArrayInputStream(privateKey.getEncoded());
        ASN1InputStream derInputStream = new ASN1InputStream(inStream);
        // PKCS1 uses ASN1, so we have to muck around with BC ASN1 sequences to get it right.
        ASN1Object keyInfo = derInputStream.readObject();
        PrivateKeyInfo pkey = new PrivateKeyInfo((ASN1Sequence) keyInfo);
        ASN1Object derKey = pkey.getPrivateKey();
        ByteArrayOutputStream bout = new ByteArrayOutputStream();
        DEROutputStream der = new DEROutputStream(bout);
        der.writeObject(derKey);
        return PEMFormatUtil.delimitBody(Base64.encodeBase64String(bout.toByteArray()), BEGIN_RSA_PRIVATE_KEY, END_RSA_PRIVATE_KEY);
*/
    }
    ...
    public static String toX509PEM(PublicKey publicKey) {
        ...
        return PEMFormatUtil.delimitBody(Base64.encodeBase64String(bytes), BEGIN_PUBLIC_KEY, END_PUBLIC_KEY);
    }
