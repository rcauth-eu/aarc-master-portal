...
public class MPOA2AutoRegistrationServlet extends OA2RegistrationServlet {
    ...
    public static String OK_PAGE = "/autoregistration-ok.jsp";
    /** Template shown when autoregistration fails */
    public static String ERROR_PAGE = "/autoregistration-error.jsp";
	...
	// TODO: Might not be needed, perhaps need other headers
        ...
        JSPUtil.handleException(t, request, response, this.ERROR_PAGE);
    }
	...
	if (((MPOA2SE)getServiceEnvironment()).getAutoRegisterEndpoint() == false)  {
     ...
     *
     * @param state {@link ClientState} to get the Client from.
     * @throws Throwable
     */
    @Override
	...
	if (state instanceof ClientState) {
	    ...
	    JSPUtil.fwd(state.getRequest(), state.getResponse(), this.OK_PAGE);
	...
	String approverid = request.getParameter(this.APPROVERID);
	if (approverid == null)	{
	    // No approverid parameter found
	    approver = this.APPROVER;
	} else {
	    // Check validity of passed approverid value
	    if (! approverid.matches(this.APPROVERID_PATTERN))  {
	    ...
	    try {
		approver = this.APPROVER + ":" + URLEncoder.encode(approverid, "UTF-8");
	    } catch (Exception e)   {
	...
	ClientApprovalStore clientApprovalStore = getServiceEnvironment().getClientApprovalStore();
