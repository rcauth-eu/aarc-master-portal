package org.masterportal.server.util;
...
public class MPTransactionConverter<V extends OA4MPServiceTransaction> extends TransactionConverter<V> {

	public MPTransactionConverter(IdentifiableProvider<V> identifiableProvider, TokenForge tokenForge,
			ClientStore<? extends Client> cs) {
		this(new DSTransactionKeys(), identifiableProvider, tokenForge, cs);

	}

	public MPTransactionConverter(SerializationKeys keys, IdentifiableProvider<V> identifiableProvider,
			TokenForge tokenForge, ClientStore<? extends Client> cs) {
	...
	public void toMap(V t, ConversionMap<String, Object> map) {
		...
		if (t.getProtectedAsset() instanceof MyX509Certificates) {
		...
		} else if (t.getProtectedAsset() instanceof MyX509Proxy) {
